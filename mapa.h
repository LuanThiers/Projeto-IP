...
void desenharMapa(Personagem perso, Texture2D mapa, int andando, int i){
    ...
    if(IsKeyPressed(KEY_K)){
        if(i == 1){
            DrawRectangle(perso.posicao.x + 100, perso.posicao.y + 60, perso.hitbox.width, perso.hitbox.height, SKYBLUE);
        }

        else{
        DrawRectangle(perso.posicao.x + 20, perso.posicao.y + 60, perso.hitbox.width, perso.hitbox.height, SKYBLUE);
        }

}
}

void desenharvilao(int time2, Texture2D vilao1, Texture2D vilao2, Texture2D vilao3, Enemy *Recvilao){
    
        if(time2>=1){
            DrawTexture(vilao1, Recvilao[3].hitbox_inimigo.x, Recvilao[3].hitbox_inimigo.y, WHITE);
            DrawTexture(vilao3, Recvilao[0].hitbox_inimigo.x, Recvilao[0].hitbox_inimigo.y, WHITE);
        }
        
        if(time2>=15){
            DrawTexture(vilao1, Recvilao[1].hitbox_inimigo.x, Recvilao[1].hitbox_inimigo.y, WHITE);
            DrawTexture(vilao2, Recvilao[4].hitbox_inimigo.x, Recvilao[4].hitbox_inimigo.y, WHITE);
        }
        
        if(time2>=40){
            DrawTexture(vilao1, Recvilao[2].hitbox_inimigo.x, Recvilao[2].hitbox_inimigo.y, WHITE);
            DrawTexture(vilao2, Recvilao[5].hitbox_inimigo.x, Recvilao[5].hitbox_inimigo.y, WHITE);
        }
}
